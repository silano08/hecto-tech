'use client'

import Link from 'next/link'
import { ExternalLink } from 'lucide-react'
import { useLanguage } from './LanguageProvider'

function XIcon({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  )
}

function LinkedInIcon({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  )
}

function GitHubIcon({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
    </svg>
  )
}

export default function Footer() {
  const currentYear = new Date().getFullYear()
  const { language } = useLanguage()
  const t = (ko: string, en: string) => language === 'ko' ? ko : en

  return (
    <footer className="mt-12 lg:mt-16 bg-black dark:bg-white -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
      {/* Main Footer Content */}
      <div className="py-10 lg:py-14">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {/* Column 1: Connect */}
          <div>
            <h3 className="font-semibold text-white dark:text-black mb-4">Connect</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link
                  href="https://www.hectofinancial.com"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('공식 홈페이지', 'Official Website')}
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="https://www.hectocareers.co.kr/join?divisions=%28%EC%A3%BC%29%ED%97%A5%ED%86%A0%ED%8C%8C%EC%9D%B4%EB%82%B8%EC%85%9C"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('채용', 'Careers')}
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="/rss.xml"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors"
                >
                  RSS
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 2: Developer Resources */}
          <div>
            <h3 className="font-semibold text-white dark:text-black mb-4">{t('개발자 리소스', 'Developer')}</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link
                  href="https://github.com/hectofinancial"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  GitHub
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="https://github.com/hectofinancial"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('오픈소스', 'Open Source')}
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="https://develop.sbsvc.online"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('API 문서', 'API Docs')}
                  <ExternalLink size={12} />
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 3: Support */}
          <div>
            <h3 className="font-semibold text-white dark:text-black mb-4">{t('고객 지원', 'Support')}</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link
                  href="https://www.hectofinancial.com/contact"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('문의하기', 'Contact Us')}
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="https://www.hectofinancial.com/faq"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  FAQ
                  <ExternalLink size={12} />
                </Link>
              </li>
            </ul>
          </div>

          {/* Column 4: Company */}
          <div>
            <h3 className="font-semibold text-white dark:text-black mb-4">{t('회사 정보', 'Company')}</h3>
            <ul className="space-y-3 text-sm">
              <li>
                <Link
                  href="https://www.hectofinancial.com/about"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('회사 소개', 'About Us')}
                  <ExternalLink size={12} />
                </Link>
              </li>
              <li>
                <Link
                  href="https://www.hectofinancial.com/news"
                  target="_blank"
                  className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors inline-flex items-center gap-1"
                >
                  {t('보도자료', 'Press')}
                  <ExternalLink size={12} />
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-gray-800 dark:border-gray-200 py-6">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          {/* Social Links */}
          <div className="flex items-center gap-4">
            <Link
              href="https://x.com/hectofinancial"
              target="_blank"
              className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors"
              aria-label="X (Twitter)"
            >
              <XIcon size={20} />
            </Link>
            <Link
              href="https://www.linkedin.com/company/hectofinancial"
              target="_blank"
              className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors"
              aria-label="LinkedIn"
            >
              <LinkedInIcon size={20} />
            </Link>
            <Link
              href="https://github.com/hectofinancial"
              target="_blank"
              className="text-gray-400 dark:text-gray-600 hover:text-primary transition-colors"
              aria-label="GitHub"
            >
              <GitHubIcon size={20} />
            </Link>
          </div>

          {/* Copyright */}
          <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-600 text-center sm:text-right">
            © {currentYear} <span className="text-primary">Hecto Financial</span>. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
  )
}
